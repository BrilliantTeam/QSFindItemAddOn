...
public class ScheduleUtil {
    ...
    public static class GLOBAL {
        ...
        public static void runTaskLater(Plugin plugin, Runnable task, long delay) {
            if (isFolia) {
                Bukkit.getGlobalRegionScheduler().runDelayed(plugin, (ignored) -> task.run(), Math.max(1L, delay));
            } else {
        ...
        public static void runTaskTimer(Plugin plugin, Runnable task, long delay, long fixedRate) {
            if (isFolia) {
                Bukkit.getGlobalRegionScheduler().runAtFixedRate(plugin, (ignored) -> task.run(), Math.max(1L, delay), Math.max(1L, fixedRate));
            } else {
        ...
        public static void runTaskLaterAsynchronously(Plugin plugin, Runnable task, long delay) {
            if (isFolia) {
                Bukkit.getAsyncScheduler().runDelayed(plugin, (ignored) -> task.run(), Math.max(1L, delay) * 50L, TimeUnit.MILLISECONDS);
            } else {
        ...
        public static void runTaskTimerAsynchronously(Plugin plugin, Runnable task, long delay, long fixedRate) {
            if (isFolia) {
                Bukkit.getAsyncScheduler().runAtFixedRate(plugin, (ignored) -> task.run(), Math.max(1L, delay) * 50L, Math.max(1L, fixedRate) * 50L, TimeUnit.MILLISECONDS);
            } else {
    ...
    public static class REGION {
        ...
        public static void runTaskLater(Plugin plugin, Chunk chunk, Runnable task, long delay) {
            if (isFolia) {
                Bukkit.getRegionScheduler().runDelayed(plugin, chunk.getWorld(), chunk.getX(), chunk.getZ(), (ignored) -> task.run(), Math.max(1L, delay));
            } else {
                Bukkit.getScheduler().runTaskLater(plugin, task, delay);
            }
        }
        ...
        public static void runTaskLater(Plugin plugin, Location location, Runnable task, long delay) {
            if (isFolia) {
                Bukkit.getRegionScheduler().runDelayed(plugin, location, (ignored) -> task.run(), Math.max(1L, delay));
            } else {
                Bukkit.getScheduler().runTaskLater(plugin, task, delay);
            }
        }
        ...
        public static void runTaskTimer(Plugin plugin, Chunk chunk, Runnable task, long delay, long fixedRate) {
            if (isFolia) {
                Bukkit.getRegionScheduler().runAtFixedRate(plugin, chunk.getWorld(), chunk.getX(), chunk.getZ(), (ignored) -> task.run(), Math.max(1L, delay), Math.max(1L, fixedRate));
            } else {
                Bukkit.getScheduler().runTaskTimer(plugin, task, delay, fixedRate);
            }
        }
        ...
        public static void runTaskTimer(Plugin plugin, Location location, Runnable task, long delay, long fixedRate) {
            if (isFolia) {
                Bukkit.getRegionScheduler().runAtFixedRate(plugin, location, (ignored) -> task.run(), Math.max(1L, delay), Math.max(1L, fixedRate));
            } else {
                Bukkit.getScheduler().runTaskTimer(plugin, task, delay, fixedRate);
            }
        }
    ...
    public static class ENTITY {
        ...
        public static void runTaskLater(Plugin plugin, Entity entity, Runnable task, long delay) {
            if (isFolia) {
                entity.getScheduler().runDelayed(plugin, (ignored) -> task.run(), null, Math.max(1L, delay));
            } else {
                Bukkit.getScheduler().runTaskLater(plugin, task, delay);
            }
        }
        ...
        public static void runTaskTimer(Plugin plugin, Entity entity, Runnable task, long delay, long fixedRate) {
            if (isFolia) {
                entity.getScheduler().runAtFixedRate(plugin, (ignored) -> task.run(), null, Math.max(1L, delay), Math.max(1L, fixedRate));
            } else {
                Bukkit.getScheduler().runTaskTimer(plugin, task, delay, fixedRate);
            }
        }
